...
export default function ReviewScreen({ shots, setShots, onGenerate, onBack }: Props) {
  ...
  // ── Crop image to 16:9 using canvas ──────────────
  ...
  const handleImageFile = async (file: File) => {
    ...
    updateShot(selected.id, {
      imageFile: file,
      imagePreview: cropped,
      status: "ready",
    });
  };

  // ── Global Ctrl+V paste listener — works on first try ──
  useEffect(() => {
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400">{readyCount} of {shots.length} ready</span>
          <div className="w-24 h-1.5 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-black rounded-full transition-all" style={{ width: `${(readyCount / shots.length) * 100}%` }} />
          </div>
          <button
            onClick={onGenerate}
            ...
            {shots.map((shot) => (
              <div
                ...
                className={`flex items-center gap-3 px-4 py-3 cursor-pointer border-b border-gray-50 transition-colors
                  ${shot.id === selectedId ? "bg-gray-50 border-l-2 border-l-black" : "hover:bg-gray-50"}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">{shot.visualDescription || "No description"}</p>
                </div>
                {shot.imagePreview && (
                  <img src={shot.imagePreview} className="w-8 h-6 object-cover rounded" alt="" />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium text-gray-600">
                    Reference Image <span className="text-gray-400 font-normal">(auto-cropped to 16:9)</span>
                  </p>
                  <p className="text-[10px] text-blue-400 mt-0.5">Press Ctrl+V anywhere to paste an image</p>
                </div>
                {selected.imagePreview && (
                  <button
                    onClick={() => updateShot(selected.id, { imageFile: null, imagePreview: null })}
              ...
              {selected.imagePreview ? (
                ...
                >
                  <img src={selected.imagePreview} alt="Reference" className="absolute inset-0 w-full h-full object-cover" />
                ...
                <div
                  ...
                  onClick={() => document.getElementById(`img-input-${selected.id}`)?.click()}
                  className={`relative w-full cursor-pointer transition-all ${imageDragging ? "bg-blue-50" : "bg-gray-50 hover:bg-gray-100"}`}
                  style={{ paddingBottom: "56.25%" }}
                >
                  <input
                    id={`img-input-${selected.id}`}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100">
                <p className="text-xs font-medium text-gray-600">Shot Details <span className="text-gray-400 font-normal">(all editable)</span></p>
              </div>
              <div className="p-4 space-y-4">
                  ...
                  <div key={key}>
                    ...
                    {multiline ? (
                      <textarea
                        ...
                        onChange={(e) => updateShot(selected.id, { [key]: e.target.value } as any)}
                        rows={3}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide">
                    Video Prompt <span className="text-blue-400 normal-case font-normal">(sent to Hailuo)</span>
                  </label>
                  <textarea
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">{(selected.prompt || "").length} characters</p>
                </div>
              </div>
            </div>
