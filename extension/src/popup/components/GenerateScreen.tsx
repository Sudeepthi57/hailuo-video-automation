  ...
  const waitForShotCompletion = (
    ...
    return new Promise((resolve) => {
      ...
      const interval = setInterval(() => {
        ...
        // Read current shots from React state — check via DOM-free approach
        // We use a ref trick: check the latest shots by requesting a state update
        setShots((prev) => {
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full p-8">
        ...
        {allDone && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-6 text-center">
            ...
            <p className="text-sm text-green-600">
              {doneCount} videos completed &middot; {errorCount} errors
            </p>
          </div>
        )}
        ...
        {!isRunning && !allDone && (
          <div className="mb-6 text-center">
            ...
            >
              Start Generating {readyShots.length} Videos &rarr;
            </button>
            <p className="text-xs text-gray-400 mt-2">
              Using <span className="font-medium text-gray-600">{selectedModel}</span> &middot; one
        ...
        {isRunning && (
          <div className="mb-6 flex items-center justify-center gap-3">
            ...
            <button
              ...
              className="text-xs text-gray-500 border border-gray-200 px-3 py-2 rounded-xl hover:bg-gray-50 transition-colors"
            >
              Stop
            </button>
          </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">
                    {shot.prompt.substring(0, 60)}...
                  </p>
                  {shot.status === 'generating' && shot.progress && (
                    <p className="text-xs text-blue-500 mt-0.5 truncate">{shot.progress}</p>
                  )}
                  {shot.status === 'error' && (
                  ...
                  {shot.status === 'done' && shot.videoUrl && (
                    <div className="flex items-center gap-2 mt-1">
                      <a
                        ...
                        download={`shot-${shot.shotNumber}.mp4`}
